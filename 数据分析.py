...
cols_corr = [ '年龄', '性别', '家庭所在地', '家庭经济情况', '学习成绩', '对待体育活动的态度',
         '剧烈运动天数', '剧烈运动单日分钟数', '适度运动天数', '适度运动单日分钟数', '步行天数',
        '步行单日分钟数', '工作日久坐分钟数', '认知', '人际', '情感', '公正', '节制', '超越', '积极心理品质得分',
        '剧烈运动一周分钟数', '适度运动一周分钟数', '步行一周分钟数', '剧烈运动MET', '适度运动MET', '步行MET', '总MET', '身体活动水平']
...
if __name__ == '__main__':
    df = pd.read_csv('data/清洗后的研究数据20220220.csv', encoding='utf-8-sig', header=0)
    cronbacha(df) #计算积极心理品质问卷的克朗巴赫系数，检验信度
    df_tmp = df[cols_corr]
    df_corr = df_tmp.corr()
    df_corr.to_csv('相关性结果20220220.csv', index=True, encoding='utf-8-sig')
